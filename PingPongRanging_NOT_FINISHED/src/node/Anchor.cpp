...
void Anchor::setConfiguration(ControlPacket rx) {
    radio_.setBandwidth(DEFAULT_BW);
    radio_.setSpreadingFactor(DEFAULT_SF);
    radio_.setFrequency(DEFAULT_RF);
...
ControlPacket Anchor::getConfiguration() {
    radio_.setBandwidth(DEFAULT_BW);
    radio_.setSpreadingFactor(DEFAULT_SF);
    radio_.setFrequency(DEFAULT_RF);
...
void Anchor::ranging(ControlPacket rx, bool master) {
    ...
    for (int i = 0; i < rx.sweepCount; i++) {
        ...
        while (chnCounter < TOTAL_CHNL) {
            ...
            int state = radio_.range(master, cfg_.targetId /*, RNG_CALIB*/);
            digitalWrite(ledPin_, LOW);

            if (master && state == RADIOLIB_ERR_NONE) {
...
bool Anchor::dataSend(ControlPacket rx) {
    connectWiFiIfNeeded();
